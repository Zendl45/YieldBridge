...
.disconnect-btn {
  ...
  color:var(--red);border:1px solid rgba(255,107,107,0.3);
...
.wallet-arrow { color:var(--muted);font-size:16px; }
...
.best-badge { position:absolute;top:10px;right:10px;background:var(--teal);color:#000;font-size:9px;font-weight:700;padding:2px 8px;border-radius:20px;text-transform:uppercase;letter-spacing:0.5px; }
...
.toast { background:rgba(10,14,23,0.95);border:1px solid var(--border);border-radius:30px;padding:10px 20px;font-size:13px;font-family:'DM Mono',monospace;backdrop-filter:blur(20px);animation:toastIn 0.3s cubic-bezier(0.34,1.56,0.64,1);white-space:nowrap; }
...
// ─── Wallet Options ───────────────────────────────────────────────────────────
const WALLETS = [
  { id: "metamask",  name: "MetaMask",             desc: "Browser extension wallet",       icon: "🦊", color: "#f6851b" },
  { id: "trust",     name: "Trust Wallet",          desc: "Fast and mobile friendly",       icon: "🛡", color: "#3375bb" },
  { id: "binance",   name: "Binance Web3 Wallet",   desc: "Best for BNB Chain",             icon: "🟡", color: "#f0b429" },
  { id: "okx",       name: "OKX Wallet",            desc: "Multi-chain support",            icon: "⚫", color: "#6b7fa3" },
];
...
export default function App() {
  ...
  async function connectWithWallet(walletId) {
    ...
    try {
      if (!window.ethereum) {
        addToast("Open this app inside your wallet's browser", "error");
        return;
      }
    ...
    try {
      ...
      if (e.code === 4902) {
        try {
          ...
          setChainOk(true);
        } catch { addToast("Failed to add BSC Testnet", "error"); }
      }
    }
  }
  ...
  const loadBalance = useCallback(async () => {
    ...
    try {
      const provider = getProvider();
      const tc = getTokenContract(provider);
      const bal = await tc.balanceOf(wallet);
  ...
  useEffect(() => {
    ...
    const onAccounts = (accs) => { if (!accs.length) disconnectWallet(); else setWallet(accs[0]); };
  ...
  async function handleDeposit() {
    ...
    try {
      ...
      if (allowance < amt) {
        ...
        const approveTx = await tc.approve(CONFIG.contracts.yieldBridge, amt * 2n);
        await approveTx.wait();
        addToast("Approved ✓", "success");
      }
      addToast("Sending deposit...", "info");
      const tx = await bc.deposit(tokenAddr, amt);
      await tx.wait();
  ...
  async function handleWithdraw() {
    ...
    try {
      ...
      const tx = await getBridgeContract(signer).withdraw(tokenAddr);
      await tx.wait();
  ...
  async function handleRebalance() {
    ...
    try {
      ...
      const tx = await getBridgeContract(signer).rebalance(tokenAddr);
      await tx.wait();
      ...
      {showWalletModal && (
        <div className="modal-overlay" onClick={() => setShowWalletModal(false)}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-sub">Choose your wallet to get started</div>
            {WALLETS.map(w => (
              <div key={w.id} className="wallet-option" onClick={() => connectWithWallet(w.id)}>
            ...
            <button className="modal-cancel" onClick={() => setShowWalletModal(false)}>Cancel</button>
          </div>
        </div>
      )}
        ...
        <nav>
          ...
          <div className="nav-right">
            ...
            <button
              ...
              onClick={() => wallet ? null : setShowWalletModal(true)}
            >
        ...
        {tab === "deposit" && (
          <div className="panel">
            ...
            <div className="token-tabs">
              {["USDT","USDC"].map(t => (
                <button key={t} className={`token-tab ${token === t ? "active" : ""}`} onClick={() => setToken(t)}>{t}</button>
              ))}
            </div>
        ...
        {tab === "position" && (
          <>
            {!wallet ? (
              ...
              <div className="empty-state">
                <div className="empty-icon">📭</div>
                No active {token} position.<br/>Go to Deposit to start earning RWA yield.
              </div>
            ) : (
              <div className="position-card">
                ...
                <div className="pos-grid">
                  ...
                  <div className="pos-item">
                    ...
                    <div className="pos-item-val">{new Date(position.depositedAt*1000).toLocaleDateString()}</div>
                  </div>
                </div>
